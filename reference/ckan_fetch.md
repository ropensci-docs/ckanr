...
- store:
  ...
  One of session (default) or disk. session stores in R session, and
  disk saves the file to disk.

- path:

  if `store="disk"`, you must give a path to store file to

- format:

  Format of the file. Required if format is not detectable through file
  URL.
...
- ...:
  ...
  `store="session"`. See details.

## Details

The `...` argument can be used to pass additional arguments to the
function that actually reads the file. It is only used when
`store="session"` and if the file is *not* a ZIP file.

This list shows what function is used to read what file format, so that
you can see what additional arguments are available:
