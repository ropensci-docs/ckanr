...
- private:
  ...
  (logical) whether the dataset is private (optional, default: `FALSE`).
  If `TRUE`, you must give a value for `owner_org`.
...
- relationships_as_object:
  ...
  (list of relationship dictionaries) See `package_relationship_create`
...
- groups:
  ...
  (data.frame) the groups to which the dataset belongs. Each row must
  have one or more of the following columns that identify an existing
