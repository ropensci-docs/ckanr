...
uses to track background operations (for example DataPusher runs).
...
- entity_type:
  ...
  (character) CKAN domain object type (for example "dataset").
...
- task_key:
  ...
  (character) Additional task key (for example queue name).
