...
- limit:
  ...
  (numeric) The maximum number of user names to return. Default: 20
  (optional)
...
A list of user dictionaries, each with the keys `name`, `fullname`, and
`id`. With `as = "table"`, a data.frame; with `as = "json"`, the raw
JSON response.
