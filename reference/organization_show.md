...
- include_datasets:
  ...
  (logical). Include a list of the organization datasets.

- include_users:

  (logical) Include the organization's users. CKAN 2.12 changed the
  server default to `FALSE` regardless of the
  `ckan.auth.public_user_details` setting
  (<https://github.com/ckan/ckan/pull/9232>); ckanr defaults to `TRUE`
  to preserve the historical behavior. The parameter is accepted on all
