...
The default base CKAN URL is <https://demo.ckan.org/>. To change it, use
[`ckanr_setup()`](https://docs.ropensci.org/ckanr/reference/ckanr_setup.md),
or use the `url` parameter in each function call.
...
with `include_datasets = TRUE` returns only the first 10 datasets. The
server caps that embedded list, and the endpoint takes no `limit`
parameter. To get every dataset of an organization, search for them with
[`package_search()`](https://docs.ropensci.org/ckanr/reference/package_search.md)
and page through the results. Each request returns at most 1000 rows, so
repeat the search with `start` until you hold them all.
...
`ckanr` provides a `dplyr` SQL interface to CKAN’s datastore. The
primary interface uses the DBI connection directly. You can access any
resource in the datastore with its CKAN resource ID.

This works only for resources in the datastore. They show the green
“Data API” button in CKAN.
...
The interface is read-only. Filter or limit large resources before you
call [`collect()`](https://dplyr.tidyverse.org/reference/compute.html),
because the DataStore returns the query result to R.

## Worked dbplyr example

This example builds a query for a resource and then sends it to CKAN.
Replace the URL, key, resource ID, and column names with values from
your CKAN site.

First create a connection and list the resources that have a DataStore
table.
...
The verbs build SQL on the server.
[`collect()`](https://dplyr.tidyverse.org/reference/compute.html) runs
the SQL and returns the rows as an R tibble. Do not use
[`copy_to()`](https://dplyr.tidyverse.org/reference/copy_to.html),
[`compute()`](https://dplyr.tidyverse.org/reference/compute.html), or
write functions with this connection. The CKAN DataStore interface is
read-only.

## Example of using a different CKAN API

See
[`ckanr::servers()`](https://docs.ropensci.org/ckanr/reference/servers.md)
for a list of CKAN servers. There are 127 as of 2020-07-29.
